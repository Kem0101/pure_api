...
async function savePublication(req: Request | any, res: Response) {
  try {
    ...
    if (!text && !file) {
      return res
        .status(400)
        .json({ msg: 'La publicación no puede estar vacía' })
    }
...
async function getPublications(req: Request | any, res: Response) {
  try {
    ...
    const follows_clean: IUser[] = follows.reduce(
      (acc: any, follow: IFollow) => {
...
async function getPublication(req: Request, res: Response) {
  try {
    ...
    if (!publication)
      return res.status(404).json({ msg: 'No existe la publicación' })
...
async function deletePublication(req: Request | any, res: Response) {
  ...
  try {
    ...
    if (!publicationRemoved) {
      return res
        .status(404)
        .json({ msg: 'Publicación no encontrada o no pertenece al usuario' })
    }

    return res
      .status(200)
      .json({ publicationRemoved, msg: 'Publicación eliminada correctamente' })
  } catch (error) {
    return res.status(500).json({ msg: 'Error interno del API' })
  }
}
