...
function deleteFollow(req: Request | any, res: Response) {
  ...
  try {
    const removed = Follow.findOneAndRemove({
      user: userId,
      followed: followId
    })
    if (!removed) {
      return res
        .status(404)
        .json({ msg: 'No se encontro el seguimiento para eliminar' })
    }
...
function getFollowingUser(req: Request | any, res: Response) {
  ...
  try {
    ...
    if (!usersFollowed || !usersFollowed.docs) {
      return res
        .status(404)
        .json({ msg: 'No estas siguiendo a ningun usuario' })
    }
...
async function getMyFollows(req: Request | any, res: Response) {
  ...
  try {
    ...
    const follows = await Follow.find(query)
      .populate({
        path: 'user',
        select: '-role -token -confirmed -_id -username -email -password -__v'
      })
      .exec()
